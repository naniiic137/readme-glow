  ...
  const items = useMemo<Item[]>(() => {
    ...
    if (current) {
      out.push(
        ...
        { id: 'viewbadge', group: 'Share', label: 'Share & access: links, badge, website, bookmarklet…', icon: 'share', keywords: 'view with readmeglow badge pretty link redirect bookmarklet website', run: run(() => openDialog('share')) },
        { id: 'gh-export', group: 'Export', label: 'GitHub export: your theme on github.com…', icon: 'github', keywords: 'svg header banner sections picture zip', run: run(() => openDialog('githubExport')) },
